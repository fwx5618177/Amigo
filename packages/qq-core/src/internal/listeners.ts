...
async function onlineListener(
    ...
    this.logger.mark(
        `加载了${this.fl.size}个好友，${this.gl.size}个群，${this.guilds.size}个频道，${this.sl.size}个陌生人`,
    );
...
function logQrcode(img: Buffer) {
    ...
    console.log(
        `${color_fg_blk + color_bg_wht}       请使用 手机QQ 扫描二维码        ${color_reset}`,
    );
    console.log(
        `${color_fg_blk + color_bg_wht}                                       ${color_reset}`,
    );
}
...
function sliderListener(this: Client, url: string) {
    this.logger.mark(
        '收到滑动验证码，请访问以下地址完成滑动，并从网络响应中取出ticket输入：' + url,
    );
