...
export abstract class Contactable {
        ...
        while (imgs.length > n) {
            ...
            for (let i = 0; i < res2.length; i++) {
                if (res2[i].status === 'rejected') {
                    res1[n + i] = res2[i];
                    this.c.logger.warn(
                        `图片${n + i + 1}上传失败, reason: ` + res2[i].reason?.message,
                    );
                }
            }
            n += 20;
    ...
    async shareMusic(platform: MusicPlatform, id: string) {
        const body = await buildMusic(
            (this.gid || this.uid) as number,
            this.dm ? 0 : 1,
            platform,
            id,
        );
    ...
    protected async _preprocess(content: Sendable, source?: Quotable) {
        try {
            ...
            const forwardNode: ForwardNode[] = content.filter(
                e => typeof e !== 'string' && e.type === 'node',
            ) as ForwardNode[];
            const task = content
                .filter(e => !forwardNode.includes(e as any))
                .map(item =>
                    typeof item === 'string' ? ({ type: 'text', text: item } as TextElem) : item,
                )
                .flat()
                .map(async elem => {
            ...
            if (source) await converter.quote(source);
    ...
    async uploadVideo(elem: VideoElem): Promise<VideoElem> {
        ...
        await new Promise((resolve, reject) => {
            exec(
                `${
                    this.c.config.ffmpeg_path || 'ffmpeg'
                } -y -i "${file}" -f image2 -frames:v 1 "${thumb}"`,
                (error, stdout, stderr) => {
                    ...
                    fs.stat(thumb, err => {
                        if (err)
                            reject(
                                new ApiRejection(
                                    ErrorCode.FFmpegVideoThumbError,
                                    'ffmpeg获取视频图像帧失败',
                                ),
                            );
    ...
    async makeForwardMsg(msglist: Forwardable[] | Forwardable): Promise<JsonElem> {
        ...
        for (const fake of msglist) {
            ...
            if (
                ...
                let fileName;
                if (elem.type === 'xml') {
                    const brief_reg = /brief\=\"(.*?)\"/gm.exec(elem.data);
                    if (brief_reg && brief_reg.length > 0) {
                        brief = brief_reg[1];
                    } else brief = '[XML]';

                    const resid_reg = /m_resid\=\"(.*?)\"/gm.exec(elem.data);
                    const fileName_reg = /m_fileName\=\"(.*?)\"/gm.exec(elem.data);
                    if (
                        resid_reg &&
                        resid_reg.length > 1 &&
                        fileName_reg &&
                ...
                if (resid && fileName) {
                    ...
                    for (const val of arr) {
                        const m_fileName = val[1].toString();
