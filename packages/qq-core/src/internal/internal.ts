...
export async function setStatus(this: Client, status: OnlineStatus) {
    ...
    if ([Platform.Watch].includes(this.config.platform as Platform)) return false;
    const d = this.device;
...
export async function getPersonalSign(this: Client): Promise<string> {
    ...
    const result = await this.sendOidbSvcTrpcTcp('OidbSvcTrpcTcp.0xfe1_2', pb.encode(body));
...
export async function getStamp(this: Client, no_cache = false) {
    ...
    if (rsp[4][1]) {
        ...
        this.stamp = new Set(
            (Array.isArray(rsp[4][1]) ? rsp[4][1] : [rsp[4][1]]).map(x => String(x)),
        );
    } else {
        this.stamp = new Set();
    }
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        this.on('internal.sso', (cmd, payload) => {
            if (cmd === 'trpc.group_pro.synclogic.SyncLogic.PushFirstView') {
                ...
                for (const p of proto[3]) {
                    const id = String(p[1]),
                        name = String(p[4]);
                    tmp.add(id);
                    if (!this.guilds.has(id)) this.guilds.set(id, new Guild(this, id));
                    const guild = this.guilds.get(id)!;
                    guild._renew(name, p[3]);
                }
                for (const [id, _] of this.guilds) {
                    if (!tmp.has(id)) this.guilds.delete(id);
                }
                clearTimeout(id);
                resolve();
            }
