...
export async function getT544(this: BaseClient, cmd: string) {
    ...
    if (this.apk.qua) {
        ...
        this.emit(
            'internal.verbose',
            `[qsign]getT544 ${cmd} result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
            VerboseLevel.Debug,
        );
        if (data.code === 0) {
            ...
            this.emit(
                'internal.verbose',
                `[qsign]签名api(energy)异常： ${cmd} result(${
                    Date.now() - time
...
export async function getSign(this: BaseClient, cmd: string, seq: number, body: Buffer) {
    ...
    if (qImei36 && this.apk.qua) {
        ...
        this.emit(
            'internal.verbose',
            `[qsign]getSign ${cmd} result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
            VerboseLevel.Debug,
        );
        if (data.code === 0) {
            ...
            this.emit(
                'internal.verbose',
                `[qsign]签名api异常： ${cmd} result(${Date.now() - time}ms): ${JSON.stringify(
                    data,
                )}`,
                VerboseLevel.Error,
            );
...
export async function requestSignToken(this: BaseClient) {
    ...
    if (qImei36 && this.apk.qua) {
        ...
        this.emit(
            'internal.verbose',
            `[qsign]requestSignToken result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
            VerboseLevel.Debug,
        );
        if (data.code === 0) {
            const ssoPacketList =
                data.data?.ssoPacketList || data.data?.requestCallback || data.data;
...
export async function submitSsoPacket(
    this: BaseClient,
    cmd: string,
    callbackId: number,
    body: Buffer,
    ...
    if (qImei36 && this.apk.qua) {
        ...
        this.emit(
            'internal.verbose',
            `[qsign]submitSsoPacket result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
            VerboseLevel.Debug,
        );
        if (data.code === 0) {
            const ssoPacketList =
...
async function register(this: BaseClient) {
    ...
    this.emit(
        'internal.verbose',
        `[qsign]register result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
        VerboseLevel.Debug,
    );
    if (data.code == 0) {
        return true;
    ...
    this.emit(
        'internal.verbose',
        `[qsign]签名api注册异常：result(${Date.now() - time}ms): ${JSON.stringify(data)}`,
        VerboseLevel.Error,
    );
    return false;
}
