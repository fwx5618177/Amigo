...
function generateImei() {
    const imei = `86${randomString(12, '0123456789')}`;
...
-----END PUBLIC KEY-----`;
    ...
    genRandomPayloadByDevice() {
        ...
        const dateFormat = (fmt?: string, time: number | Date = Date.now()) =>
            formatTime(time, fmt);
        ...
        const beaconIdArr: (string | number)[] = [
            `${formatTime(new Date(timestamp + fixedRand(60, 0)))}.${String(
                fixedRand(99, 0),
            ...
            fixedRand(5, 0),
            `${dateFormat('YYYY-mm-ddHHMMSS')}.${String(
                (10 + parseInt(this.imei.slice(5, 7))) % 100,
            ).padStart(2, '0')}0000000`,
            `${dateFormat('YYYY-mm-ddHHMMSS')}.${String(
                (11 + parseInt(this.imei.slice(5, 7))) % 100,
            ).padStart(2, '0')}0000000`,
            fixedRand(10000, 1000),
            fixedRand(100, 10),
            `${dateFormat('YYYY-mm-ddHHMMSS')}.${String(
