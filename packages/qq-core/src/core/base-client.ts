...
export class BaseClient extends Trapper {
    ...
    protected signLoginCmd = ['wtlogin.login', 'wtlogin.exchange_emp'];
    protected signCmd = [
    ...
    async ssoPacketListHandler(list: Packet[] | null) {
        ...
        if (!this.isOnline()) {
            ...
            if (this.ssoPacketList.length > 0) {
                ...
                this.ssoPacketList = [];
                for (const val of list) {
                    const ssoPacket: any = list1.find((data: any) => {
        ...
        for (const ssoPacket of list) {
            ...
            const body = Buffer.from(ssoPacket.body as unknown as string, 'hex');
            const callbackId = ssoPacket.callbackId as number;
            const payload = await this.sendUni(cmd, body);
            this.emit(
                'internal.verbose',
                `sendUni ${cmd} result: ${payload.toString('hex')}`,
                VerboseLevel.Debug,
            );
            if (callbackId > -1) {
    ...
    calcPoW(data: any) {
        ...
        if (hashType !== 1) {
            this.emit(
                'internal.verbose',
                `Unsupported tlv546 hash type ${hashType}`,
                VerboseLevel.Warn,
            );
        ...
        switch (typ) {
            case 1:
                ...
                this.emit(
                    'internal.verbose',
                    `Unsupported tlv546 algorithm type ${typ}`,
                    VerboseLevel.Warn,
                );
                break;
            case 2:
                ...
                if (tgt.length === 32) {
                    ...
                    while (Buffer.compare(hash, tgt) !== 0) {
                        ...
                        if (cnt > 6000000) {
                            this.emit(
                                'internal.verbose',
                                'Calculating PoW cost too much time, maybe something wrong',
                                VerboseLevel.Error,
                            );
                            throw new Error(
                                'Calculating PoW cost too much time, maybe something wrong',
                    ...
                    this.emit(
                        'internal.verbose',
                        `Calculating PoW of plus ${cnt} times cost ${elp} ms`,
                        VerboseLevel.Debug,
                    );
                }
    ...
    async tokenLogin(token: Buffer) {
        ...
        try {
            ...
            this.emit(
                'internal.verbose',
                '旧版token于当前版本不兼容，请手动删除token后重新运行',
                VerboseLevel.Error,
            );
        ...
        try {
            ...
            if (!result && !reflush) {
                ...
                this[HEARTBEAT] = setInterval(async () => {
                    ...
                    const heartbeat_cmd = [Platform.Tim].includes(this.config.platform as Platform)
                        ? 'OidbSvc.0x480_9'
                    ...
                    this.sendUni(heartbeat_cmd, this.sig.hb480)
                        ...
                        .then(async () => {
                            await this[FN_SEND](
                                await buildLoginPacket.call(this, 'Heartbeat.Alive', BUF0, 0),
                                5,
                            ).catch(() => {
                                this.emit(
                                    'internal.verbose',
                                    'Heartbeat.Alive timeout',
                                    VerboseLevel.Warn,
                                );
                            });
...
async function refreshToken(this: BaseClient) {
    if (!this.isOnline() || timestamp() - this.sig.emp_time < 43000) return;
    const t = tlv.getPacker(this);
    const tlv_count = 16;
    const writer = new Writer()
        .writeU16(11)
...
function decodeLoginResponse(this: BaseClient, payload: Buffer): any {
    ...
    if (type === 235) {
        ...
        return this.emit(
            ...
            type,
            `[登陆失败](${type})当前设备信息被拉黑，建议删除"${device_path}"后重新登录！`,
        );
    }
    ...
    if (type === 237) {
        return this.emit(
            'internal.error.login',
            type,
            `[登陆失败](${type})当前QQ登录频繁，暂时被限制登录，建议更换QQ或稍后再尝试登录！`,
        );
    }
