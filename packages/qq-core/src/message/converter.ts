...
export class Converter {
    ...
    private at(elem: AtElem) {
        ...
        if (qq === 'all') {
            (q = 0), (flag = 1), (display = '全体成员');
        } else {
            (q = Number(qq)), (flag = 0), (display = text || String(qq));
            if (!text) {
    ...
    private music(elem: MusicElem) {
        if (
            (['title', 'singer', 'jumpUrl', 'musicUrl', 'preview'] as (keyof MusicFullInfo)[]).some(
                key => !elem[key],
            )
        )
            return this.json({
        ...
        this.elems.push({
            51: {
                1: Buffer.concat([
                    BUF1,
                    deflateSync(
                        typeof elem.data === 'string' ? elem.data : JSON.stringify(elem.data),
                    ),
                ]),
            },
        });
    ...
    private reply(elem: ReplyElem) {
        ...
        if (id.length > 24)
            this.quote({
                ...parseGroupMessageId(id),
                message: elem.text || '[消息]',
            });
        else
            this.quote({
                ...parseDmMessageId(id),
