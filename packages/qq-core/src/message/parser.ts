...
export class Parser {
    ...
    private parseExclusiveElem(type: number, proto: pb.Proto) {
        ...
        switch (type) {
            ...
            case 37: //qlottie
                ...
                if (proto[2][2]) elem.qlottie = String(proto[2][2]);
    ...
    private parseImgElem(proto: pb.Proto, type: 'flash' | 'image') {
        ...
        if (
            proto[34] &&
            proto[34][30] &&
            String(proto[34][30]).startsWith('&rkey=') &&
            this.imgprefix
        ) {
            // QQNT图片
            elem = {
            ...
            proto[29] &&
            proto[29][30] &&
            String(proto[29][30]).startsWith('&rkey=') &&
            this.imgprefix
        ) {
            // QQNT图片
            elem = {
        ...
        if (proto[7]?.toHex) {
            ...
            else if (proto[15]) elem.url = `https://c2cpicdw.qpic.cn${proto[15]}`;
            ...
            elem = {
                ...
                url: proto[16]
                    ? `https://gchat.qpic.cn${proto[16]}`
                    : getGroupImageUrl(proto[13].toHex()),
            };
