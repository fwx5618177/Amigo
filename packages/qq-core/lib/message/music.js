...
async function getQQSong(id) {
    ...
    const mid = rsp.mid, title = rsp.name, album = rsp.album.mid, singer = rsp.singer?.[0]?.name || 'unknown';
...
async function get163Song(id) {
    let rsp = await axios_1.default.get(`http://music.163.com/api/song/detail/?id=${id}&ids=[${id}]`, {
        responseType: 'json',
    });
...
async function getMiGuSong(id) {
    ...
    let preview = '';
    try {
        const a = await axios_1.default.get(`https://music.migu.cn/v3/api/music/audioPlayer/getSongPic?songId=${rsp.songId}`, {
            responseType: 'json',
    ...
    const url = await axios_1.default.get(`https://app.c.nf.migu.cn/MIGUM2.0/v1.0/content/shareInfo.do?contentId=${rsp.contentId}&contentName=${encodeURIComponent(rsp.songName)}&resourceType=2&targetUserName=${encodeURIComponent(rsp.singer)}`, { responseType: 'json' });
...
async function getKuwoSong(id) {
    ...
    const url = await axios_1.default.get(`http://www.kuwo.cn/api/v1/www/music/playUrl?mid=${id}&type=music&httpsStatus=1`);
    return {
        title: rsp.name,
        singer: rsp.artist,
