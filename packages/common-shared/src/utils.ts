...
> = T extends new (...args: infer A) => infer L
    ? O extends new (...args: any[]) => infer R
        ? new (...args: [...A, ...any[]]) => MergeType<L, R>
        : new (...args: any[]) => L
    : T;
type MergedManyClass<T extends new (...args: any[]) => any, O extends any[]> = O extends [
    infer L,
    ...infer R,
]
...
export function Mixin<
    T extends new (...args: any[]) => any,
    O extends (new (...args: any[]) => any)[],
>(base: T, ...classes: O): MergedManyClass<T, O> {
...
export const evaluate = <S, T = any>(exp: string, context: S) =>
    execute<S, T>(`return(${exp})`, context);
...
export function toJSON(cls: object): Dict {
    ...
    const { info = {}, ...obj } = Object.fromEntries(
        getProperty(cls)
            .filter(key => {
                return (
                    typeof cls[key] !== 'function' &&
                    !key.startsWith('_') &&
                    !['c', 'client'].includes(key)
                );
            })
            .map(key => {
                return [key, cls[key]];
