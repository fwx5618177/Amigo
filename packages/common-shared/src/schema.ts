export class Schema<S = any, T = S> {
    ...
    constructor(public meta: Schema.Meta<S, T>, public options: Schema.Options = {}) {
        const _this = this;
    ...
    static fromJSON<S, T>(json: Schema.JSON<S, T>) {
        ...
        if (dict)
            options.dict = Object.fromEntries(
                Object.entries(dict).map(([key, value]) => [key, Schema.fromJSON(value)]),
            );
        ...
        return Object.fromEntries(
            Object.entries({
                ...
                dict: this.options.dict
                    ? Object.fromEntries(
                          Object.entries(this.options.dict || {}).map(([key, value]) => [
                              key,
                              value.toJSON(),
                          ]),
                      )
...
export namespace Schema {
    ...
    export function checkDefault<T>(schema: Schema, value: T, fallback: T = value) {
            ...
            switch (schema.meta.type) {
                case 'string':
                    if (!['string', 'undefined'].includes(typeof value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a string`);
                    break;
                case 'number':
                    if (!['number', 'undefined'].includes(typeof value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a number`);
                    break;
                case 'boolean':
                    if (!['boolean', 'undefined'].includes(typeof value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a boolean`);
                    break;
                case 'regexp':
                    if (
                        ...
                        throw new TypeError(`${schema.meta.key || 'value'} is not a RegExp|string`);
                    break;
                case 'date':
                    if (!['number', 'undefined'].includes(typeof value) && !(value instanceof Date))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a Date|number`);
                    if (value instanceof Date && isNaN(value.getTime()))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a valid Date`);
                    break;
                case 'object':
                case 'dict':
                    if (!['object', 'undefined', 'null'].includes(typeof value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a object`);
                    break;
                case 'array':
                case 'list':
                    if (typeof value !== 'undefined' && !Array.isArray(value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a list`);
                    break;
                case 'tuple':
                    if (typeof value !== 'undefined' && !Array.isArray(value))
                        throw new TypeError(`${schema.meta.key || 'value'} is not a valid tuple`);
                    break;
                case 'union':
                    if (
                        typeof value !== 'undefined' &&
                        !schema.options.list?.some(item => {
                            try {
                                item(value);
                                return true;
                            } catch {
                ...
                case 'intersect':
                    if (
                        typeof value !== 'undefined' &&
                        !schema.options.list?.every(item => {
                            try {
                                item(value);
                                return true;
                            } catch {
                        ...
                        throw new TypeError(
                            `${schema.meta.key || 'value'} is not a valid intersect`,
                        );
                    break;
                case 'const':
                    if (typeof value !== 'undefined' && value !== schema.meta.default)
                        throw new TypeError(`${schema.meta.key || 'value'} is not const`);
                    break;
                case 'percent':
                    if (
                        typeof value !== 'undefined' &&
                        ...
                        throw new TypeError(`${schema.meta.key || 'value'} is not a valid percent`);
                    break;
                case 'any':
                    break;
                case 'never':
        ...
        if (schema.meta.required && typeof value === 'undefined')
            throw new Error(`${schema.meta.key || 'value'} is required`);
        validateType(schema, value);
        return value;
    }
    export type Option<T = any> = T extends Array<infer R>
        ?
              | R
              | {
                    label: string;
...
Schema.extend('number', function (this: Schema, value: any) {
    ...
    if (this.meta.max && value > this.meta.max)
        throw new Error(`${this.meta.key || 'value'} is too large`);
    if (this.meta.min && value < this.meta.min)
        throw new Error(`${this.meta.key || 'value'} is too small`);
    return value;
});
Schema.extend('string', function (this: Schema, value: any) {
    value = Schema.checkDefault(this, value);
    if (this.meta.max && value?.length > this.meta.max)
        throw new Error(`${this.meta.key || 'value'} is too long`);
    if (this.meta.min && value?.length < this.meta.min)
        throw new Error(`${this.meta.key || 'value'} is too short`);
    return value;
});
...
Schema.extend('object', function (this: Schema, value: any) {
    value = Schema.checkDefault(this, value, {});
    return Object.fromEntries(
        Object.entries(this.options.dict).map(([key, schema]) => {
